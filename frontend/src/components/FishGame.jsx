        ...
        >
          ...
          {gameState === 'playing' && (
            <div className="absolute top-12 left-4 text-white text-sm sm:text-base sm:top-16 sm:left-6 z-10 bg-black bg-opacity-40 rounded-lg p-3">
              ...
              <div className="text-xs sm:text-sm opacity-75">Difficulty: {Math.min(Math.floor(score / 20) + 1, 30)}</div>
            </div>
          )}
          ...
          <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center rounded-lg p-4">
            <Card className="p-4 sm:p-8 text-center bg-blue-900 border-blue-700 max-w-sm w-full">
              ...
              <div className="space-y-3">
                ...
                <Button 
                  onClick={async () => {
                    // Determine achievement badge and title
                    const achievement = score >= 1000 ? { emoji: '👑', title: 'Ocean Deity', color: '#FFD700' } :
                                       score >= 700 ? { emoji: '🌌', title: 'Abyssal Master', color: '#9333EA' } :
                                       score >= 500 ? { emoji: '🌟', title: 'Legendary Swimmer', color: '#EF4444' } :
                                       score >= 300 ? { emoji: '🐠', title: 'Fish Whisperer', color: '#10B981' } :
                                       score >= 200 ? { emoji: '⭐', title: 'Deep Sea Explorer', color: '#F59E0B' } :
                                       score >= 100 ? { emoji: '🥇', title: 'Gold Swimmer', color: '#FFD700' } :
                                       score >= 50 ? { emoji: '🥈', title: 'Silver Swimmer', color: '#C0C0C0' } :
                                       score >= 20 ? { emoji: '🥉', title: 'Bronze Swimmer', color: '#CD7F32' } :
                                       { emoji: '🐟', title: 'Novice Swimmer', color: '#3B82F6' };
                    ...
                    ctx.fillText('🐠 Seaweed Swimmer', 600, 100);
                    
                    // Score box
                    ctx.fillStyle = 'rgba(0, 0, 0, 0.5)';
                    ctx.roundRect(200, 180, 800, 280, 20);
                    ctx.fill();
                    
                    // "My Score" text
                    ctx.fillStyle = '#93c5fd';
                    ctx.font = '40px Arial';
                    ctx.fillText('My Score', 600, 240);
                    
                    // Large score number
                    ctx.fillStyle = '#ffffff';
                    ctx.font = 'bold 120px Arial';
                    ctx.fillText(score.toString(), 600, 360);
                    
                    // "seconds" text
                    ctx.fillStyle = '#93c5fd';
                    ctx.font = '32px Arial';
                    ctx.fillText('seconds', 600, 400);
                    
                    // Achievement badge emoji (large)
                    ctx.font = '80px Arial';
                    ctx.fillText(achievement.emoji, 600, 500);
                    
                    // Achievement title
                    ctx.fillStyle = achievement.color;
                    ctx.font = 'bold 40px Arial';
                    ctx.fillText(achievement.title, 600, 550);
                    
                    // Call to action at bottom
                    ctx.fillStyle = '#ffffff';
                    ctx.font = '28px Arial';
                    ctx.fillText('Can you beat my score?', 600, 600);
                    
                    // Convert canvas to blob
                    canvas.toBlob(async (blob) => {
                      const shareText = `I just scored ${score} seconds in Seaweed Swimmer! 🐠🌊 ${achievement.title}! Can you beat my score?`;
                      ...
                      if (navigator.share && navigator.canShare && navigator.canShare({ files: [new File([blob], 'score.png', { type: 'image/png' })] })) {
                        try {
                          const file = new File([blob], 'seaweed-swimmer-score.png', { type: 'image/png' });
                      ...
                      link.href = url;
                      link.download = 'seaweed-swimmer-score.png';
                      link.click();
