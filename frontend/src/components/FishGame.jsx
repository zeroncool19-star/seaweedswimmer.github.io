...
const FishGame = () => {
  ...
  const initGame = useCallback(() => {
    ...
    game.gameStartDelay = 5000; // 3 second countdown + 2 second grace period before physics start
  ...
  const jumpFish = useCallback(async () => {
    if (gameState === 'playing') {
      // Prevent jumping during the entire start delay period (countdown + grace period)
      const gameRunningTime = Date.now() - gameRef.current.startTime;
      const inStartDelay = gameRunningTime < gameRef.current.gameStartDelay;
      
      if (inStartDelay) {
        return; // Block all taps during countdown AND grace period
      }
  ...
  const gameLoop = useCallback(() => {
    ...
    if (inStartDelay) {
      ...
      // Only show countdown during first 3000ms (3, 2, 1), hide during grace period (last 2000ms)
      if (remainingDelay > 2000) {
        const countdownSeconds = Math.ceil((remainingDelay - 2000) / 1000);
        if (countdownSeconds !== countdown) {
          setCountdown(countdownSeconds);
        }
      } else if (countdown !== 0) {
        setCountdown(0); // Hide countdown during grace period
      }
    } else if (countdown > 0) {
      setCountdown(0);
    }
