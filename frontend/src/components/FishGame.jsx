...
const FishGame = () => {
  ...
  const jumpFish = useCallback(async () => {
    if (gameState === 'playing') {
      ...
      const currentDifficulty = Math.floor(score / 60) + 1;
  ...
  const gameLoop = useCallback(() => {
    ...
    if (gameStarted) {
      ...
      // Update difficulty every 60 points and increase fish speed
      game.difficulty = Math.floor(newScore / 60) + 1;
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-blue-600 to-blue-800 p-2 sm:p-4">
      <div className="relative max-w-full">
        ...
        <div className="absolute top-2 left-2 text-white text-sm sm:text-base sm:top-4 sm:left-4">
          ...
          <div className="text-xs sm:text-sm opacity-75">Difficulty: {Math.floor(score / 60) + 1}</div>
        </div>
